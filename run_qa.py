...
def load_and_cache_examples(args, tokenizer, evaluate=False):
    ...
    cached_features_file = os.path.join(data_dir, 'cached_seq{}_{}'.format(
        str(args.max_seq_length),
        filename_base
        ))
    ...
    if os.path.exists(cached_features_file):
        ...
        fn = processor.get_examples
        examples = fn(filename)
...
def main():
    ...
    args.model_num_outputs = num_labels
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            global_step = checkpoint.split('-')[-1] if len(checkpoints) > 1 and len(checkpoint.split('-')) > 1 else ""
            model = model_class.from_pretrained(checkpoint)
            model.output_hidden_states = True
            model.output_attentions = True
            model.to(args.device)
            result = evaluate(args, model, tokenizer, prefix=global_step, analyze_attentions=True, eval_on_train=False, epoch=global_step)
